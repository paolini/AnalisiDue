...
$df_x\colon \RR^n \to \RR$ e dunque ad ogni $v\in \RR^n$ associa un numero reale 
$df_x[v]$. 
Se sfruttiamo il prodotto scalare standard su $\RR^n$ 
possiamo identificare l'applicazione lineare $df_x$ con il vettore $\nabla f(x)$:
\[
...
\begin{definition}[forme differenziali lineari]
    ...
    che ad ogni punto $x\in A$ associano una applicazione lineare $\omega_x\colon \RR^n \to \RR$,
    $v\mapsto \omega_x[v]$.
\end{definition}

\begin{remark}
Le forme differenziali sono funzioni di due variabili: $(x,v)\mapsto \omega_x[v]$.
...
per questo le due variabili vengono posizionate in modo diverso: 
una come pedice, una tra parentesi quadre.
...
nella forma 
\[
   \omega = \xi_1 dx_1 + \xi_2 dx_2 + \dots + \xi_n dx_n.
\]
...
\section{integrale di una forma differenziale lineare}
...
\begin{equation}\label{def:integrale_forma_1}
 \int_u \omega \defeq \int_a^b \omega_{u(t)}[u'(t)]\, dt.
\end{equation}
Osserviamo che se $\omega$ si rappresenta con il campo vettoriale $\xi$, 
si ha $\omega_x[v] = \xi(x)\cdot v$ 
e dunque 
\[
...
\begin{align*}
  ...
  &= \int_A^B \omega_{u(\phi(s))}[(u\circ \phi)'(s)]\, ds
  = \int_A^B \omega_{u(\phi(s))}[u'(\phi(s))]\, \phi'(s)\, ds\\
  &= \int_a^b \omega_{u(t)}[u'(t)]\, dt = \int_u \omega.
\end{align*}
...
tramite la formula
\[
   (\phi^* \omega)_x[v] = \omega_{\phi(x)}[d\phi_x[v]].
\]
...
\[
  (u^* \omega)_t[e_1] = \omega_{u(t)}[u'(t)]\, dt.
\]
...
\begin{align*}    
    \int_u \omega 
    &= \int_u df 
    = \int_a^b df_{u(t)}[u'(t)]\, dt 
    = \int_a^b \nabla f(u(t)) \cdot u'(t) \, dt\\
    &= \int_a^b (f(u(t)))' \, dt 
    = f(u(b)) - f(u(a)).
\end{align*}
...
Sia $\omega_{(x,y)} = y dx$, $\omega \in \Omega^1(\RR^2)$ è una forma di classe $C^\infty$ 
...
(cioè esiste $x_0\in A$ tale che per ogni $y\in A$ si ha $[x_0,y]\subset A$). 
...
%
\begin{proof}
Senza perdita di generalità possiamo supporre $x_0 = 0 \in A$.
Sia $\xi$ il campo vettoriale che rappresenta $\omega$.
Definiamo $f\colon A\to\RR$ come l'integrale di $\omega$ lungo il segmento 
che congiunge $0$ ad $x$:
\[
   f(x) = \int_{[0,x]} \omega = \int_0^1 \xi(tx)\cdot x\, dt 
   = \int_0^1 \sum_j x_j \xi_j(tx) \, dt.
\]
Ricordiamo che se $\omega$ è chiusa si ha 
$\frac{\partial \xi_j}{\partial x_k} = \frac{\partial \xi_k}{\partial x_j}$.
Dunque
\begin{align*}
 \frac{\partial}
