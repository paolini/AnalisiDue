...
allora $f+g$, $f^2$, $g^2$ e $(f+g)^2$ lo sono
(vedi appunti di Analisi Matematica Uno).
...
\[
  C^0_0([0,2\pi]) 
  = \ENCLOSE{u\in C^0([0,2\pi])\colon u(0)=u(2\pi)=0}
  \subset C^0([0,2\pi]).
\]
\end{theorem}
%
\begin{proof}
Si tratta di applicare il teorema di Stone-Weiestrass 
alla famiglia $\mathcal A$ dei polinomi trigonometrici:
verifichiamo le ipotesi del teorema.
Chiaramente $\mathcal A$ è un'algebra in quanto i polinomi 
lo sono.
Le costanti sono in $\mathcal A$ in particolare $1\in \mathcal A$.
Dati $x,y \in [0,2\pi)$ se $x\neq y$ 
si ha certamente $(\cos x, \sin x) \neq (\cos y, \sin y)$
e dunque o $\sin$ o $\cos$ assumono valori diversi nei due punti.
\end{proof}
