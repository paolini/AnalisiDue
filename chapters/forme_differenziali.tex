...
sono il differenziale di una funzione $f\colon \Omega\to \RR$.
...
$f\in C^1(\Omega)$ tale che $\omega = df$ allora $\omega$ è chiusa. 
\end{theorem}
...
Sia $\omega(x,y) = y dx$, $\omega \in \Lambda^1(\RR^2)$ è una forma di classe $C^\infty$ 
...
Si consideri $\Omega=\RR^2\setminus\ENCLOSE{(0,0)}$.
Sia $\omega\in \Lambda^1(\Omega)$ 
\[
...
Prendiamo invece $u\colon [0,2\pi]\to \Omega$ la circonferenza
di raggio $\rho$:
$u(t) = \rho (\cos t,\sin t)$.
...
perché $\Omega$ è un esempio di aperto non stellato.
Osserviamo che se da $\Omega$ rimuoviamo l'asse delle $y$,
la funzione 
\[
