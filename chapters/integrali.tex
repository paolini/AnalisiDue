...
% 
% Allora, per ogni insieme PJ-misurabile $E\subset [0,1]^n$, $\phi(E)$ è misurabile e si ha
...
allora $\phi(\Omega)$ è PJ-misurabile.
...
\[
  \partial \phi(\Omega) 
    \subset \overline{\phi(\Omega)} \setminus \phi(\Omega)
    \subset \phi(\bar \Omega) \setminus \phi(\Omega) 
    \subset \phi(\partial \Omega).
\]
...
Visto che $\phi$ è di classe $\C^1$
sul compatto $\bar \Omega$, 
esiste $L\in \RR$ tale che $\abs{D\phi(\vec x)} \le L$ per ogni $\vec x \in \bar \Omega$.
Essendo $\Omega$ convesso 
possiamo applicare il teorema~\ref{th:lagrange} di Lagrange 
...
per ottenere la misurabilità di $\phi(\Omega)$ e di $\phi(\bar \Omega)$.
Lo stesso lemma ci dice anche che $\partial \phi(\Omega) \subset \phi(\partial \Omega)$.
% \[
%   m^*(\partial (\phi(\Omega)))
...
deve esistere un punto $\vec z\in [\vec q, \vec y]\cap \partial \phi(\Omega)$
...
contenuti interamente in $\Omega$ e $\Omega^M$ il compatto unione 
di tutti i cubetti che intersecano $\partial \Omega$.
Così $\abs{\Omega^M} = m^M(\bar \Omega)$ e $\abs{\Omega_M} = m_M(\Omega)$.

Per il Lemma~\ref{lm:misura-immagine-lipschitz} si ha 
\[
 m^*(\phi(\Omega^M \setminus \Omega_M)) 
...
per ogni $\vec k\in \ZZ^n$ con $Q^N_{\vec k} \subset \Omega_M$ e 
per ogni $\vec x, \vec x' \in Q^N_{\vec k}\cap \Omega$ si ha
\[
  \Abs{D\phi(\vec x) - D\phi(\vec x')} \le \frac{\eps}{L \sqrt n},
  \qquad
...
\[
\abs{\rule{0pt}{1.1em}\abs{\phi(Q^N_{\vec k})} - 2^{-Nn} \abs{\det M^N_{\vec k}}}
...
= \abs{\rule{0pt}{1.1em}\abs{\phi(Q^N_{\vec k})} - 2^{-Nn} \abs{\det D\phi}}
