...
($1$-forma).
Si tratta di una funzione che ad ogni punto $x\in A$ associa una forma lineare 
...
\begin{definition}[forme differenziali lineari]
    ...
    Denotiamo con $A^1(A)$ l'insieme di tutte le funzioni $\omega\colon A \to (\RR^n)^*$
    che ad ogni punto $x\in A$ associano una forma lineare $\omega(x)\colon \RR^n \to \RR$.
\end{definition}
...
\[
  ...
  \end{cases}  
\]
In effetti per ogni $p\in A$ le applicazioni lineari 
...
corrispondenza le forme differenziali lineari $\omega \in A^1(A)$ 
...
e $\omega = df$, il vettore che rappresenta $\omega$ è $\grad f$:
\[
...
\end{align*}
Dunque se rappresentiamo $\omega\in \Omega^2(A)$, $A\subset\RR^3$ tramite 
...
\end{equation}
In tal caso il campo vettoriale che rappresenta $d\omega$ sarà
$\rot \xi$.
