...
%
\begin{proof}
In base al criterio della radice
il raggio di convergenza della serie è il reciproco 
del limite superiore di $\sqrt[k]{a_k}$. 
Visto che $\sqrt[k]{k} \to 1$, la serie originaria 
e la serie delle derivate hanno lo stesso raggio di
convergenza.
Nel teorema precedente abbiamo mostrato che su ogni 
intervallo $[-r,r]$ con $r<R$ la serie di potenze 
con somma $f$ converge totalmente. 
Dunque converge uniformemente e possiamo scambiare 
la derivata con la somma per ottenere $f'(x) = g(x)$.
\end{proof}
...
Per il Lemma di Abel (vedi appunti di Analisi Matematica Uno)
sappiamo che la funzione $f(x)$ è continua nel punto $x=-1$ 
