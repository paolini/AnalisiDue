...
$a,b\in \RR$ sono sequenzialmente compatti. 
...
Allora $K$ è sequenzialmente compatto.
\end{theorem}
\begin{proof}
...
Visto che $K$ è chiuso deve essere $\vec p\in K$ e dunque $K$ è sequenzialmente compatto.
\end{proof}

In uno spazio metrico generico non è detto che gli insiemi chiusi e limitati 
siano sequenzialmente compatti (un esempio negativo è dato dalla convergenza uniforme, come
vedremo più avanti). 
...
\begin{theorem}%
\mymark{**}%
Se $A$ è un sottoinsieme sequenzialmente compatto 
...
\begin{theorem}[Weierstrass: le funzioni continue mandano 
...
Essendo $K$ sequenzialmente compatto possiamo estrarre una sottosuccessione convergente: $x_{k_j}\to x$. 
Essendo $f$ continua si ha
\[
  y_{k_j} = f(x_{k_j}) \to f(x) \in f(K).
\]
\end{proof}

Nel caso $X=Y=\RR$ recuperiamo l'usuale teorema di Weierstrass, 
in quanto se $f\colon [a,b]\to \RR$ è continua essendo $[a,b]$ sequenzialmente compatto 
risulta che $f([a,b])$ è sequenzialmente compatto. 
Ma gli insiemi sequenzialmente compatti di $\RR$ sono chiusi e limitati 
quindi hanno 
massimo e minimo in quanto l'estremo superiore e l'estremo inferiore 
sono finiti e sono punti di aderenza dell'insieme.
...
Sia $f\colon X\to Y$ una funzione continua. 
Allora $f$ è uniformemente continua.
\end{theorem}
%
\begin{proof}
Supponiamo per assurdo che $f\colon X\to Y$ sia una funzione continua 
ma non uniformemente continua. Dunque non vale la condizione di uniforme
continuità:
\[
 \forall \eps>0\, \exists \delta>0, \forall x,y\in X\colon 
  d(x,y)<\delta \implies d(f(x),f(y))< \eps. 
\]
La negazione di tale condizione è 
(ricordiamo che la negazione inverte i quantificatori 
e la negazinoe dell'implicazione $P\implies Q$ è $P\land \lnot Q$):
\[
 \exists \eps>0\, \forall \delta>0, \exists x,y\in X\colon
 d(x,y)<\delta \land d(f(x),f(y))\ge \eps.
\]
Fissato $\eps>0$ applichiamo la condizione precedente 
con $\delta = \frac 1 k$ per ogni $k\in \NN$ in modo da ottenere 
due successioni $x_k$, $y_k$ tali che
\[
  d(x_k,y_k)< \frac 1 k, \qquad d(f(x_k),f(y_k))\ge eps.
\]
Per la sequenziale compattezza di $X$ esiste una estratta $j_k$ 
tale che $x_{k_j}\to \bar x$ con $\bar x\in X$.
Visto che $d(x_k,y_k)\to 0$ si ha anche $y_{k_j}\to \bar x$.
Dalla continuità di $f$ si ottiene $f(x_{k_j})\to f(\bar x)$ 
e $f(x_{k_j})\to f(\bar x)$ da cui $d(f(x_{k_j}),f(y_{k_j}))\to 0$.
Ma questo è in contraddizione con la condizione $d(f(x_k),f(y_k))\ge \eps$.
\end{proof}
