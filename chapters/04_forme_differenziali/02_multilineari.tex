...
\[
   ...
   \defeq \sum_{\sigma\in S_k} (-1)^\sigma \xi_1[v_{\sigma(1)}]\cdots \xi_k[v_{\sigma(k)}]
\]
...
\[
  \lambda_{\hat k} \defeq (-1)^k dx_{1} \wedge \dots \wedge dx_{k-1} \wedge dx_{k+1} \wedge \dots \wedge dx_n.
\]
In pratica $dx_k$ è l'unico differenziale mancante in $\lambda_{\hat k}$.
Nell'esempio precedente avevamo $k=3$, $x_1=x$, $x_2=y$, $x_3=z$,
$\lambda_{\hat 1} = dy\wedge dz$, 
$\lambda_{\hat 2} = dz\wedge dx$ e $\lambda_{\hat 3} = dx \wedge dy$.

Dunque se $\omega \in \Omega^{n-1}(A)$, $A\subset \RR^n$, si può scrivere:
\[
 \omega = \sum_{i=1}^n (-1)^i \x_i dx_1 \wedge \dots \wedge dx_{i-1} \wedge dx_{i+1} \wedge \dots \wedge dx_{n}
\]
con $\xi\colon A \to \RR^n$.
\end{example}
