\section{funzioni di più variabili}

Uno degli argomenti principali del secondo corso di Analisi Matematica è lo studio 
delle funzioni di più variabili. 
Il nostro ambiente di studio privilegiato sarà 
lo spazio vettoriale reale di dimensione $n$. 
Si considera assimilata la conoscenza degli spazi vettoriali di dimensione 
finita. 
In particolare si considera come fatto noto che ogni spazio 
vettoriale reale di dimensione $n$ è isomorfo a $\RR^n$. 
Spesso lo spazio vettoriale sarà naturalmente dotato di prodotto scalare (spazio euclideo) e,
anche in questo caso, sappiamo che è possibile identificare il prodotto scalare 
dello spazio euclideo con il prodotto scalare canonico di $\RR^n$.
Dal prodotto scalare si ottiene la norma euclidea, che induce una metrica e quindi una topologia.

Riprendiamo brevemente questi concetti.
\begin{definition}
L'insieme $\RR^n$ è l'insieme delle $n$-uple (ordinate) di numeri reali 
$\vec x = (x_1,\dots,x_n)$.
Le operazioni di addizione e di moltiplicazione per uno scalare $t\in \RR$
sono definite come segue:
\begin{align*}
\vec x + \vec y &= (x_1+y_1,\dots,x_n+y_n)\\
t \vec x &= (t x_1,\dots,t x_n).
\end{align*}
Con queste operazioni $\RR^n$ risulta essere uno spazio vettoriale reale di dimensione $n$.

Possiamo definire il prodotto scalare canonico di $\RR^n$ come segue:
\[
\vec x \cdot \vec y = x_1 y_1 + \dots + x_n y_n.
\]
La norma euclidea di $\vec x$ è definita come
\[
\abs{\vec x} = \sqrt{\vec x\cdot \vec x} = \sqrt{x_1^2 + \dots + x_n^2}.
\]
\end{definition}

Quando $n=2$ o $n=3$ può essere comodo (e certamente è usuale) nominare 
le coordinate di $\RR^2$ come $x$ e $y$ e le coordinate di $\RR^3$ come $x$, $y$ e $z$
(invece che $(x_1,x_2)$ e $(x_1,x_2,x_3)$).

Considereremo funzioni con dominio in $A\subset \RR^n$ e codominio $\RR^m$. 
Scriveremo $\vec f\colon A \subset \RR^n \to \RR^m$.
Avremo quindi $\vec y = \vec f(\vec x)$ con $\vec x =(x_1,\dots,x_n)\in A$ e $\vec y = (y_1,\dots,y_m)\in \RR^m$.
Nel caso $m=1$ la funzione si dice scalare, 
mentre nel caso $m>1$ la funzione si dice vettoriale.
Nel caso $n=1$ la funzione si dice di una variabile, 
mentre nel caso $n>1$ la funzione si dice di 
più variabili ($n$ variabili per la precisione).
Una funzione vettoriale $\vec f$ può essere identificata con una 
$m$-upla di funzioni scalari $f_1,\dots,f_m$ definite su $A$:
\[
  \vec f(x_1, \dots, x_n) = \begin{pmatrix}
    f_1(x_1,\dots, x_n) \\
    \vdots \\
    f_m(x_1,\dots, x_n)
  \end{pmatrix}.
\]

Se $I\subset \RR$ è un intervallo, 
una funzione $\vec u\colon I\subset \RR \to \RR^n$ si chiama 
\emph{curva}. 
Se pensiamo a $t\in I$ come il \emph{tempo}, la funzione 
$t\mapsto \vec u(t)$ rappresenta la dinamica di un punto 
che si muove nello spazio $\RR^n$.
L'immagine della funzione $\vec u(I) = \ENCLOSE{\vec u(t)\colon t\in I}$
si chiama \emph{traiettoria} del punto $\vec u(t)$ o \emph{supporto}
della curva $\vec u$.
A volte si denota con $[\vec u]$.

Se ad esempio fissiamo un punto $\vec x_0\in \RR^n$ 
e un vettore $\vec v\in \RR^n$, la curva $\vec u(t) = \vec x_0 + t \vec v$
rappresenta il moto rettilineo uniforme di un punto che al tempo $t=0$ 
si trova in posizione $x_0$ e che si muove con velocità $\vec v$.
Il supporto di $\vec u$ è in questo caso una retta passante per $x_0$ 
con direttrice $\vec v$.

Per ricondursi allo studio di funzioni di una variabile 
è spesso utile studiare la composizione della funzione di più variabili 
con opportune curve. 
Se $\vec f\colon A \subset \RR^n\to \RR^m$ e $\vec u\colon I\subset \RR\to A\subset \RR^n$ 
possiamo considerare la funzione $\vec g\colon I \to \RR^m$ 
data da $\vec g(t) = \vec f(\vec u(t))$. 
Si avrà $\vec g = (g_1,\dots, g_m)$ con $g_k(t) = f_k(\vec u(t))$
dove $\vec f = (f_1,\dots, f_m)$. 
In definitiva le funzioni $g_k\colon I\subset \RR \to \RR$ 
sono funzioni scalari di una variabile, e le possiamo studiare 
come si è fatto nel corso di Analisi Matematica Uno.

\begin{example}[funzioni lineari]
Fissato $\vec w \in \RR^n$ consideriamo la funzione 
lineare $f \colon \RR^n \to \RR$ definita da 
\[
   f(\vec x) = (\vec x , \vec w).
\]  
Fissato $\vec q\in \RR^n$ e $\vec v \in \RR^n$ consideriamo la retta (parametrizzata da)
$\vec u(t) = t \vec v + \vec q$.
Si avrà allora 
\[
  f(\vec u(t)) = (t \vec v + \vec q, \vec w) = t (\vec v,\vec w) + (\vec q, \vec w).
\]
Si osserva ad esempio che se $\vec v$ è ortogonale a $\vec w$,
cioè $(\vec v, \vec w)=0$, si ha $f(\vec u(t))= (\vec q, \vec v)$ è costante.
Abbiamo trovato una \emph{curva di livello}, cioè una curva su 
cui $f$ risulta costante.
\end{example}


\section{continuità e limiti}
La norma euclidea definisce una metrica e quindi una topologia 
su $\RR^n$. 

\begin{definition}[topologia]
Dato $\vec x \in \RR^n$ e $\rho>0$, la palla aperta di raggio $\rho$ centrata in $\vec x$ è
\[
B_\rho(\vec x) = \{\vec y \in \RR^n : \abs{\vec y - \vec x} < \rho\}.
\]

Se esiste $\rho>0$ per cui $B_\rho(\vec x) \subset A$, diremo che $\vec x$ è un punto interno di $A$
e diremo che $A$ è un intorno di $\vec x$.
Possiamo dunque definire la famiglia di intorni di $\vec x$ come segue:
\[
  \mathcal U_x = \ENCLOSE{U\subset \RR^n\colon \exists \rho>0\colon B_\rho(\vec x) \subset U}.
\]

La \emph{parte interna} di $A$ è l'insieme dei punti interni di $A$.
Un sottoinsieme $A\subset \RR^n$ si dice aperto se 
è intorno di ogni suo punto. 

Un sottoinsieme $A\subset \RR^n$ si dice chiuso se il suo complementare $\RR^n\setminus A$ è aperto.

Un punto $\vec x\in \RR^n$ si dice essere un punto di accumulazione per $A$ 
se ogni intorno di $\vec x$ contiene almeno un punto di $A$ diverso da $\vec x$.
\end{definition}

Una successione $\vec a_k\in \RR^n$ è una funzione $\vec a \colon \NN\to \RR^n$.
Ogni punto $\vec a_k$ è una $n$-upla: $\vec a_k = ((a_k)_1,\dots, (a_k)_n) \in \RR^n$.

Una successione $\vec a_k$ converge ad un punto $\vec x\in \RR^n$ 
se $\abs{\vec a_k - \vec x}\to 0$.
Scriveremo in tal caso $\vec a_k \to \vec x$ (per $k\to +\infty$).
Notiamo che $\abs{\vec a_k-\vec x}$ è una successione di numeri reali 
non negativi, e dunque l'abbiamo già studiata nel corso di \emph{Analisi Matematica Uno}.
In particolare, se espandiamo la definizione di limite, si vede 
che $\vec a_k \to \vec x$ significa:
\[
 \forall \eps>0\, \exists N\in \NN\, \forall k\ge N\colon \abs{\vec a_k- \vec x} < \eps.
\]

\begin{definition}[continuità]
Sia $f\colon A \subset \RR^n \to \RR^m$.
Fissato $x_0 \in A$ diremo che $f$ è \emph{continua} nel punto 
$x_0$
se 
\[
\forall \eps>0\, \exists \delta>0 \, \forall x\in A\colon
  \abs{x - x_0} < \delta \implies \abs{f(x) - f(x_0)} < \eps.
\] 

Una funzione si dice essere \emph{sequenzialmente continua} nel punto 
$x_0$ se per ogni successione $a_k\in A$ con $a_k\to x_0$
si ha $f(a_k)\to f(x_0)$.

Una funzione $f\colon A \subset \RR^n \to \RR^m$ si dice essere 
continua (o sequenzialmente continua) se è continua (o sequenzialmente 
continua) in ogni punto $x\in A$.
\end{definition}

\begin{theorem}[equivalenza tra continuità e continuità sequenziale]
Sia $f\colon A\subset \RR^n\to \RR^m$ e sia $x_0\in A$.
Allora $f$ è continua in $x_0$ se e solo se $f$ è sequenzialmente 
continua in $x_0$.
\end{theorem}
\begin{proof}
La dimostrazinoe è la stessa già vista nel corso di Analisi Matematica Uno.
\end{proof}

\begin{theorem}[continuità della funzione composta]
Sia $f\colon A\subset \RR^n \to B \subset \RR^k$ una funzione 
continua (nel punto $x_0\in A$) e sia $g\colon B\subset \RR^k \to \RR^m$ 
una funzione continua (nel punto $y_0=f(x_0)\in B$).
Allora la composizione $g\circ f \colon A \subset \RR^n \to \RR^m$ 
è continua (nel punto $x_0$).
\end{theorem}
%
\begin{proof}
La dimostrazione è la stessa già vista nel corso di Analisi Matematica Uno.
\end{proof}



\section{differenziale}


\begin{theorem}[continuità delle applicazioni lineari]
  \label{th:lineare_continua}
Ogni applicazinoe lineare $L\colon \RR^n\to \RR^m$ 
è continua.
\end{theorem}
%
\begin{proof}
Sia $\vec e_1,\dots, \vec e_n$ la base canonica di $\RR^n$.
Consideriamo
\[
  M = \max\ENCLOSE{\abs{L[\vec e_1]}, \dots, \abs{[L\vec e_n]}}.
\]
Si ha allora, per ogni $\vec x\in \RR^n$,
\[
  \abs{L[\vec x]}
  =\abs{L[x_1 \vec e_1 + \dots + x_n \vec e_n]}
  \le \abs{x_1 L[\vec e_1]} + \dots + \abs{x_n L[\vec e_n]}
  \le nM\abs{\vec x}.
\]
Dunque per $\vec x \to 0$ si ha $\abs{L[\vec x]}\to 0$.
Significa che $L$ è continua in $0$.

Ma in ogni altro punto si ha 
\[
  L[\vec x] - L[\vec x_0] = L[\vec x - \vec x_0] 
\]
e dunque se $\vec x\to \vec x_0$ si ottiene che $L[\vec x]\to L[\vec x_0]$.
\end{proof}

\begin{definition}[norma operatoriale]
Sia $L\colon \RR^n \to \RR^m$ una applicazione lineare.
Definiamo la \emph{norma} di $L$ come:
  \[
  \Abs{L} = \sup \ENCLOSE{\abs{L[\vec x]}\colon \abs{\vec x}\le 1}.
  \]
\end{definition}

\begin{theorem}[proprietà della norma operatoriale]
Sia $L\colon \RR^n \to \RR^m$ una applicazione lineare. 
Allora 
\begin{enumerate}
\item $\Abs L < +\infty$;
\item $\Abs L$ è la costante ottimale per cui vale 
la stima:
\begin{equation}
  \label{eq:stima_norma_operatoriale}  
  \abs{L[\vec v]} \le \Abs L \cdot \abs {\vec v},
  \qquad \forall \vec v \in \RR^n,
\end{equation}

inoltre esiste $\vec v\in\RR^n$ per cui vale 
l'uguaglianza;
\item se $M\colon \RR^m \to \RR^k$ è un'altra applicazinoe 
lineare si ha 
\[
  \Abs{M\circ L} \le \Abs M \cdot \Abs L.
\]
\end{enumerate}
\end{theorem}
%
\begin{proof}
Per il teorema~\ref{th:lineare_continua} la funzione $\abs{L}$ è continua,
dunque per il teorema di Weiestrass ammette massimo sul disco
unitario $\ENCLOSE{\vec x \in \RR^n\colon \abs{x}\le 1}$.
Dunque $\Abs{L}\in \RR$ è finito ed è realizzato 
su un vettore di norma unitaria.
Se $t\in \RR$ e $\vec v$ è unitario si ha
\[
  \abs{L[t\vec v]} = \abs{t}\cdot \abs{L[\vec v]}
  \le \abs{t}\cdot \Abs L \cdot \abs{\vec v} 
  = \Abs L \cdot \abs{t\vec v}
\]
da cui si ottiene~\ref{eq:stima_norma_operatoriale}.
Per l'ultimo punto osserviamo che per ogni $\vec v\in \RR^n$ si ha 
\[
  \Abs{ML[\vec v]} = \Abs{M[L[\vec v]]}
  \le \Abs{M}\cdot \abs{L[\vec v]}
  \le \Abs{M}\cdot \Abs{L}\abs{\vec v}.
\]
Passando al $\sup$ sui $\vec v$ unitari si ottiene il risultato.
\end{proof}

\begin{definition}[differenziabilità]
  Sia $\vec f\colon A \subset \RR^n \to \RR^m$ e sia $\vec x_0\in A$.
  Diremo che $f$ è differenziabile in $\vec x_0$ se esiste 
  una applicazione lineare $d \vec f_{\vec x_0}\colon \RR^n \to \RR^m$ tale che 
  \begin{equation}\label{eq:def_differenziale}
    \vec f(\vec x) = \vec f(\vec x_0) + d\vec f_{\vec x_0}[\vec x- \vec x_0] + o(\abs{\vec x - \vec x_0}), 
    \qquad \text{per } \vec x\to \vec x_0.
  \end{equation}
  L'applicazione lineare $d \vec f_{\vec x_0}$ si 
  chiama \emph{differenziale} di $\vec f$ nel punto $\vec x_0$.
\end{definition}

\begin{theorem}[proprietà delle funzioni differenziabili]
Sia $\vec f\colon A\subset \RR^n \to \RR^m$ una funzione 
differenziabile nel punto $\vec x_0\in A$. Allora
\begin{enumerate}
  \item $\vec f$ è continua in $\vec x_0$;
  \item se $\vec x_0$ è un punto interno di $A$
  il differenziale è unico
\end{enumerate}
\end{theorem}
\begin{proof}
Visto che le applicazioni lineari sono continue
si ha $d\vec f_{\vec x_0}[\vec x - \vec x_0]\to 0$ per $\vec x \to \vec x_0$
e dunque dalla definizione~\eqref{eq:def_differenziale} 
si ottiene che $\vec f(\vec x) - \vec f(\vec x_0)\to \vec 0$
per $\vec x \to \vec x_0$. 

Siano ora $L$ e $M$ due differenziali di $\vec f$ in $\vec x_0$
e sia $\vec v$ un qualunque vettore unitario.
Prendendo la formula~\eqref{eq:def_differenziale}
prima con $L$ e poi con $M$ al posto di $d \vec f_{\vec x_0}$,
e facendone la differenza, si ottiene
\[
  (L-M)[\vec x - \vec x_0] = o(\abs{\vec x-\vec x_0}), 
  \qquad \text{per } \vec x \to \vec x_0.
\]
Preso $\vec x = \vec x_0 + t\vec v$ osserviamo che essendo $\vec x_0$ interno 
ad $A$ avremo che per $t$ sufficientemente piccolo si ha effettivamente 
$\vec x \in A$. 
Dunque, per $t\to 0$ si ha $\vec x \to \vec x_0$ e quindi
\[
  (L-M)[t \vec v] = o(\abs{t}\cdot \abs{\vec v}) = o(t) = t o(1)
\] 
da cui 
\[
  (L-M)[\vec v] = o(1).
\]
Visto che il lato sinistro non dipende da $t$ 
significa che $(L-M)[\vec v]=0$ per ogni $\vec v$ unitario.
Questo significa che $L-M = 0$ ovvero $L=M$.
\end{proof}

La precedente definizione poteva ugualmente essere data 
per funzioni definite tra due spazi normati astratti.
Se lo spazio di partenza non avesse avuto dimensione finita 
si richiede usualmente che il differenziale $d\vec f_{\vec x_0}$ 
sia continuo (oltre che lineare), cosa che risulta automatica
nel caso di dimensione finita.

L'applicazione lineare $d\vec f_{\vec x_0}\colon \RR^n\to\RR^m$ 
può essere rappresentata tramite una matrice $m\times n$. 
Tale matrice si chiama \emph{matrice Jacobiana} e si indica 
di solito con $D\vec f(\vec x_0)$ (oppure $J_{\vec f}(\vec x_0)$ o anche 
$\nabla f(\vec x_0)$)
Si ha quindi
\[
  d \vec f_{\vec x_0} [\vec v] = Df(\vec x_0) \vec v,
  \qquad \text{per ogni } \vec v\in \RR^n.
\]

Nel caso $m=1$, cioè $f\colon A\subset \RR^n\to \RR$, la matrice Jacobiana
è formata da una singola riga e si identifica quindi con un 
vettore di $\RR^n$. 
Tale vettore si chiama \emph{gradiente} di $f$ e si indica con 
$\vec\nabla f(\vec x_0)$. 
Si ha dunque
\[
  d f_{\vec x_0}[\vec v] = \langle \vec\nabla f(\vec x_0), \vec v\rangle.
\]

In generale si può scrivere $\vec f = (f_1,\dots,f_m)$ con $f_k\colon \RR^n \to \RR$
e si troverà quindi che le righe della matrice $D\vec f$ jacobiana di $\vec f$
sono i vettori gradiente $\vec\nabla f_1, \dots, \vec\nabla f_m$.

Nel caso $n=m=1$, cioè $\vec f\colon A\subset \RR\to\RR^m$, $x_0\in A$,
la matrice Jacobiana è formata da una singola colonna che si denota con
$\vec f'(x_0) \in \RR^n$.
Il differenziale si scrive nella forma $d\vec f_{x_0}[h] = h \vec f'(x_0)$.
La definizione di differenziale diventa
\[
 \vec f(x) = \vec f(x_0) + \vec f'(x_0)\cdot (x-x_0) + o(x-x_0)
\] 
che è equivalente a 
\[
  \frac{\vec f(x) - \vec f(x_0)}{x-x_0} = \vec f'(x_0) + o(1)
\]
cioè la definizione di derivata vista nel corso di Analisi Matematica Uno.
Per le funzioni di una variabile \emph{differenziabile} è quindi equivalente 
a \emph{derivabile}. 

\begin{theorem}[differenziale della composizione]
  Sia $\vec f\colon A \subset \RR^n \to B\subset \RR^k$,
  $\vec g\colon B\subset \RR^k \to \RR^m$, $x_0\in A$.
  Se $\vec f$ è differenziabile in $\vec x_0$ 
  e $\vec g$ è differenziabile in $\vec y_0 = \vec f(\vec x_0)$ 
  allora la funzione composta $\vec g\circ \vec f\colon A \subset \RR^n \to \RR^m$
  è differenziabile in $\vec x_0$ e si ha 
  \[
    d (\vec g \circ \vec f)_{\vec x_0} 
    = d\vec g_{\vec y_0}\circ d\vec f_{\vec x_0}.
  \]
\end{theorem}
%
\begin{proof}
  Facendo il cambio di variabile $\vec y = \vec f(x)$ nella formula~\eqref{eq:def_differenziale}
  per $\vec g$
  e inserendo la formula stessa per $\vec f$
  si ottiene
  \begin{align*}
    \vec g(\vec f(\vec x)) 
    & = \vec g(\vec y_0) 
        + d\vec g_{\vec y_0}[\vec f(\vec x)-\vec f(\vec x_0)]
        + o(\abs{\vec f(\vec x)- \vec f(\vec x_0)})\\
    & = \vec g(\vec f(\vec x_0)) 
        + d\vec g_{\vec x_0}[d\vec f_{\vec x_0}[\vec x - \vec x_0] 
          + o(\abs{\vec x - \vec x_0})]
        + o(\abs{d\vec f_{\vec x_0}[\vec x - \vec x_0] 
          + o(\abs{\vec x - \vec x_0})})\\
    & = \vec g(\vec f(\vec x_0))
       + d\vec g_{\vec x_0}[d\vec f_{\vec x_0}[\vec x - \vec x_0]]\\
    & \quad + (\Abs{d \vec g_{\vec x_0}}+\Abs{d \vec f_{\vec x_0}} + 1)\cdot o(\abs{\vec x - \vec x_0})
  \end{align*}
  che è la condizione di differenziabilità per $\vec g \circ \vec f$ 
  nel punto $\vec x_0$ con differenziale $d \vec g_{\vec y_0}\circ d \vec f_{\vec x_0}$.
\end{proof}

Nel caso particolare in cui 
$\vec f\colon A\subset \RR^n \to \RR^m$ e $\vec u\colon I\subset \RR\to A\subset \RR^n$,
la formula per la derivata della funzione composta diventa
\[
  \frac{d}{dt} \vec f(\vec u(t))
  = d\vec f_{\vec u(t)}[\vec u'(t)]
  = D\vec f(\vec u(t))\vec u'(t)
\]
che nel caso $m=1$ si può anche scrivere usando il gradiente
\[
  \frac{d}{dt} f(\vec u(t))
  = \langle \vec\nabla f(\vec u(t)),\vec u'(t)\rangle.
\]

Dato $\vec v\in\RR^n$ possiamo considerare la retta con direzione $\vec v$
passante per un punto $\vec x\in \RR^n$: $\vec u(t) = \vec x + t\vec v$.
Definiamo allora la \emph{derivata direzionale} di $\vec f$ nel punto $\vec x$
in direzione $\vec v$ come:
\[
   \frac{\partial \vec f}{\partial \vec v}(\vec x)
   = \Enclose{\frac {d}{dt} \vec f(\vec u(t))}_{t=0}.
\]
E' possibile che la derivata direzionale esista anche se la funzione $\vec f$ 
non è differenziabile, perché questa definizione coinvolge i valori della funzinoe 
solamente sulla retta fissata.
Ma se $\vec f$ è differenziabile in $\vec x$, visto che $\vec u(t)=\vec x + t\vec v$ 
è derivabile con derivata $\vec u'(t) = \vec v$ si ha 
\[
  \frac{\partial \vec f}{\partial \vec v} (\vec x)
  = d\vec f_{\vec x}[\vec v]
  = D\vec f(\vec x) \vec v.
\]
E nel caso scalare $m=1$:
\[
  \frac{\partial f}{\partial \vec v} (\vec x)
  = \langle \vec\nabla f(\vec x), \vec v\rangle.
\]

Si capisce quindi che per determinare il differenziale di una funzione 
differenziabile è sufficiente 
calcolare le derivate direzionali negli $n$ vettori di una base.
In particolare se si scelgono i vettori della base canonica si ottengono
le cosiddette \emph{derivate parziali}:
\[
   \frac{\partial \vec f}{\partial x_k} 
   = \frac{\partial \vec f}{\partial \vec e_k}
   = \Enclose{\frac{d}{dt} \vec f(\vec x + t\vec e_k)}_{t=0}.
\]
Se poi scriviamo $\vec f = (f_1,\dots,f_m)$ possiamo quindi 
determinare tutte le componenti della matrice jacobiana:
\[
  D\vec f(\vec x)_{jk} = D f_j(\vec x) \vec e_k
  = \langle \vec\nabla f_j(\vec x),\vec e_k\rangle
  = (d f_j)_{\vec x}[\vec e_k]
  = \frac{\partial f_j}{\partial x_k}(\vec x).
\]
Dunque 
\[
  D\vec f(\vec x) = \begin{pmatrix}
    \frac{\partial f_1}{\partial x_1} & \dots & \frac{\partial f_1}{\partial x_n}\\
    \vdots & \ddots & \vdots \\
    \frac{\partial f_m}{\partial x_1} & \dots & \frac{\partial f_m}{\partial x_n}\\
  \end{pmatrix}
\]
e nel caso $m=1$ 
\[
  \vec\nabla f (\vec x) = \begin{pmatrix}
    \frac{\partial f}{\partial x_1}\\
    \vdots\\
    \frac{\partial f}{\partial x_n}
  \end{pmatrix}.
\]

Si può anche usare una notazione più sintetica che utilizza 
gli indici per indicare le derivate parziali:
\[
   f_{x_k} = \frac{\partial f}{\partial x_k}.
\]

Le derivate parziali sono uno strumento molto 
efficace per calcolare le componenti del differenziale 
di una funzione di più variabili.
Il problema è che l'esistenza delle derivate parziali 
non garantisce la differenziabilità.
Risulta quindi molto utile il seguente.

\begin{theorem}[del differenziale]
Sia $A\subset \RR^n$ aperto 
e sia $\vec f\colon A \to \RR^m$ una funzione 
tale che per ogni $\vec x\in A$ esistono 
tutte le derivate parziali $\partial f_k/\partial x_j$
(si dice a volte: la funzione è \emph{derivabile}).
Supponiamo inoltre che le derivate parziali siano 
tutte continue.
Allora $\vec f$ è differenziabile in ogni punto. 
\end{theorem}
\begin{proof}
\emph{Passo 1:} facciamo il caso $n=2$, $m=1$.
Scriveremo $(x,y)$ al posto di $\vec x$ 
e $(x_0,y_0)$ al posto di $\vec x_0$.
Il vettore obliquo $(x,y)-(x_0,y_0)$ 
può essere decomposto nella somma dei 
due vettori paralleli agli assi coordinati
$(x,y)-(x_0,y)$ e $(x_0,y)-(x_0,y_0)$.
Alla funzione ristretta 
alle direzioni orizzontale e verticale possiamo
applicare il teorema di Lagrange per ottenere
\begin{align*}
  f(x,y) - f(x_0,y_0)
  = f(x,y) - f(x_0,y) + f(x_0,y) - f(x_0,y_0) \\
  = f_x(\tilde x,y)\cdot (x-x_0)
    + f_y(x_0,\tilde y)\cdot (y-y_0)
\end{align*}
con $\tilde x \in (x_0,x)$ e $\tilde y\in(y_0,y)$.
I valori di $\tilde x$ e $\tilde y$ dipendono 
da $x$ e da $y$, ma tendono rispettivamente 
a $x_0$ e $y_0$ quando $(x,y)\to (x_0,y_0)$.
Vogliamo dimostrare che $f$ è differenziabile in $(x_0,y_0)$.
Se lo è, necessariamente il suo differenziale
si scriverà nella forma $df[h,k] = h f_x + k f_y$.
La formula~\eqref{eq:def_differenziale}
diventa quindi per $(x,y)\to(x_0,y_0)$
\begin{align*}
  \lefteqn{f(x,y) - f(x_0,y_0) 
  - f_x(x_0,y_0)\cdot (x-x_0) - f_y(x_0,y_0)\cdot (y-y_0)}\\
  &= \Enclose{f_x(x_0,y_0) - f_x(\tilde x, y)}(x-x_0)\\
  & \quad + \Enclose{f_y(x_0,y_0) - f_y(x_0,\tilde y)}(y-y_0)\\
  & = o(1) (x-x_0) + o(1) (y-y_0)
\end{align*}
in quanto per l'ipotesi di continuità delle derivate parziali
le parentesi quadre tendono a zero quando $(x,y)\to (x_0,y_0)$.
Osservando infine che 
$\abs{x-x_0}, \abs{y-y_0}\le \sqrt{(x-x_0)^2 + (y-y_0)^2}$
si ottiene il risultato voluto.

\emph{Passo 2:} $n>2$, $m=1$.
Si procede esattamente come nel passo 1, salvo che per 
passare dal punto $\vec x_0$ al punto $\vec x$ bisogna 
seguire $n$ segmenti paralleli agli assi su ognuno 
dei quali si applica il teorema di Lagrange.

\emph{Conclusione.} Se $m>1$ basta applicare il risultato 
alle componenti $f_1,\dots, f_m$ di $\vec f$ e osservare 
che la formula \eqref{eq:def_differenziale} è equivalente 
alle $m$ formule:
\[
  f_k(\vec x) = f_k(\vec x_0) + (d f_k)_{\vec x_0}[\vec x-\vec x_0] + o(\abs{\vec x-\vec x_0}).
\]
\end{proof}

\section{derivate successive}

In generale possiamo definire il differenziale di una funzione 
$f\colon V\to W$
definita tra spazi vettoriali normati.
Il differenziale risulta essere allora 
una funzione $df\colon V \to \L(V,W)$ che manda 
ogni punto $x\in V$ in cui la funzione è differenziabile
nell'applicazione lineare $df_x\colon V\to W$.
Lo spazio $\L(V,W)$ risulta a sua volta essere uno spazio normato
(dalla norma operatoriale). 
E' quindi possibile iterativamente differenziare il differenziale.

Nel caso in cui $W=\RR$ il differenziale 
$df\colon V \to V'$ ha valori nello spazio duale di $V = \L(V,\RR)$
che, tramite prodotto scalare,
può essere messo in corrispondenza con $V$ stesso. 
Il vettore di $V$ che rappresenta il differenziale è il gradiente $\nabla f$:
$df_{\vec x}[v] = \langle \nabla f, v\rangle$.
Il differenziale secondo è quindi una funzione $d^2 f\colon V\to \L(V,V')$ 
che può essere rappresentato tramite una applicazione bilineare 
$D^2 f$ (o $\nabla^2 f$) per cui si ha
\[
   d^2 f_x[v][w] = \langle D^2 f(x) v, w\rangle.
\]
La matrice associata a $D^2 f$ si chiama \emph{matrice hessiana}
(a volte si denota con $Hf$). 

Per le funzioni a valori in $\RR^m$ e per le derivate successive 
alla seconda la notazione si complica ulteriormente.
Conviene in tal caso usare i \emph{multi-indici} per indicare tutte le 
componenti delle derivate successive. 
Per ogni $j=1,\dots,m$ e per $\vec i = i_1,\dots, i_k = 1,\dots, n$ 
si considerano le derivate parziali:
\[
    f^{(\vec i)}_j = \frac{\partial^k f_j}{\partial x_{i_1} \dots \partial x_{i_k}}.
\] 

\begin{definition}[classi di regolarità]
Sia $A\subset \RR^n$ un aperto. 
Definiamo $C^0(A,\RR^m)$ l'insieme delle funzioni continue
$\vec f\colon A \to \RR^m$. 
Per induzione si definisce la classe $C^k(A,\RR^m)$ 
delle funzioni che hanno tutte le prime $k$ derivate parziali 
continue:
\[
   C^k(A,\RR^m) = \ENCLOSE{\vec f\colon A\to\RR^m\Coloneq
     \vec f_{x_1}, \dots, \vec f_{x_n} \in C^{k-1}(A,\RR^m)
   }.
\]

Si intende poi $C^k(A) = C^k(A,\RR)$.
\end{definition}

\begin{theorem}[Schwarz]
Sia $A\subset \RR^n$ un aperto 
e sia $f\colon A \subset \RR^n \to \RR$.
Fissati $k,j\in \ENCLOSE{1,\dots,n}$
supponiamo che in tutti i punti di $A$ esistano le derivate parziali 
$\frac{\partial f}{\partial x_k}$ 
e $\frac{\partial f}{\partial x_j}$
e che a loro volta esistano le derivate seconde miste 
$\frac{\partial^2 f}{\partial x_j \partial x_k}$
e $\frac{\partial^2 f}{\partial x_k \partial x_j}$.
Supponiamo infine che le derivate seconde miste siano continue 
in un punto $\vec x_0\in A$. Allora sono uguali:
\[
 \frac{\partial^2 f}{\partial x_j \partial x_k}(\vec x_0)
 = \frac{\partial^2 f}{\partial x_k \partial x_j}(\vec x_0).
\]
\end{theorem}
%
\begin{proof}
  \emph{Passo 1:} supponiamo $n=2$ e usiamo le variabili $x,y$ invece che 
  $x_1,x_2$. 
L'idea è che le derivate seconde miste approssimano l'incremento che si ottiene 
per andare dal punto $(x_0,y_0)$ al punto $(x,y)$ seguendo due diversi percorsi,
il primo incrementando prima la $x$ e poi la $y$ e il secondo 
incrementando prima la $y$ e poi la $x$.
Consideriamo quindi la seguente identità:
\[
\Enclose{f(x,y)-f(x_0,y_0)} - \Enclose{f(x,y_0) - f(x_0,y_0)}
= \Enclose{f(x,y) - f(x,y_0)} - \Enclose{f(x_0,y) - f(x_0, y_0)}
\]
e applichiamo il teorema di Lagrange 
a sinistra alla funzione $f(x,\cdot) - f(x_0,\cdot)$ nell'intervallo $[y_0,y]$ 
e a destra alla funzinoe $f(\cdot,y) - f(\cdot,y_0)$ nell'intervallo $[x_0,x]$.
Si ottiene quindi
\[
\Enclose{\frac{\partial f}{\partial y}(x,\tilde y)-\frac{\partial f}{\partial y}(x_0,\tilde y)}(y-y_0)
= \Enclose{\frac{\partial f}{\partial x}(\tilde x,y)-\frac{\partial f}{\partial x}(\tilde x,y_0)}(x-x_0)
\]
per qualche $\tilde x \in (x_0,x)$ e $\tilde y \in (y_0,y)$.
Applichiamo ora nuovamente il teorema di Lagrange a sinistra
sull'intervallo $[x_0,x]$ e a destra sull'intervallo $[y_0,y]$ 
per ottenere
\[
\frac{\partial^2 f}{\partial x\partial y}(\hat x,\tilde y)(x-x_0)(y-y_0)
= \frac{\partial^2 f}{\partial y\partial x}(\tilde x,\hat y)(y-y_0)(x-x_0)
\]
ovvero
\[
\frac{\partial^2 f}{\partial x\partial y}(\hat x,\tilde y)
= \frac{\partial^2 f}{\partial y\partial x}(\tilde x,\hat y)
\]
per qualche $\hat x \in (x_0,x)$ e $\hat y \in (y_0,y)$.

Per $(x,y)\to(x_0,y_0)$ si ha $\tilde x, \hat x \to x_0$ e $\tilde y, \hat y\to y_0$
e grazie all'ipotesi di continuità delle derivate seconde miste 
passando al limite si conclude:
\[
\frac{\partial^2 f}{\partial x\partial y}(x_0, y_0)
= \frac{\partial^2 f}{\partial y\partial x}(x_0, y_0).  
\]

\emph{Caso generale.}
Consideriamo la funzione di due variabili ottenuta da $f$ fissando 
tutte le coordinate diverse da $j$ e $k$ ai valori delle corrispondenti 
coordinate del punto $\vec x_0$. 
Si può quindi applicare il passo precedente.
\end{proof}

\begin{theorem}[Formula di Taylor]
Sia $A\subset \RR^n$ aperto e 
sia $f\colon A\subset \RR^n \to \RR$ una funzione di classe $C^2$.
Allora fissato $\vec x_0\in A$ si ha, per $\vec x \to \vec x_0$
\begin{equation}\label{eq:taylor_secondo}
  f(\vec x) = f(\vec x_0) + \langle \vec\nabla f(\vec x_0), \vec x - \vec x_0\rangle 
  + \frac 1 2 \langle D^2 f(\vec x_0) (\vec x - \vec x_0), \vec x - \vec x_0\rangle
  + o(\abs{\vec x - \vec x_0}^2).
\end{equation}

Più in generale, se $f$ è di classe $C^m$ si avrà
\[
 f(\vec x) = f(\vec x_0) + \sum_{k=1}^m \frac 1 {k!} d^k f_{\vec x_0}[\vec x-\vec x_0]^k
  + o(\abs{\vec x - \vec x_0}^k).
\]
\end{theorem}
%
\begin{proof}
  \emph{Caso $m=2$ (secondo ordine)}.
Fissato $\vec x$ (oltre che $\vec x_0$) consideriamo 
il vettore $\vec v = \vec x - \vec x_0$ e restringiamo la funzione 
alla retta passante per $\vec x_0$ con direzione $\vec v$.
Otteniamo una funzione 
\[
  g(t) = f(\vec x_0 + t \vec v)
\]
le cui derivate sono:
\begin{align*}
  g'(t) &= \langle \nabla f(\vec x_0 + t\vec v), \vec v\rangle,\\
  g''(t) &= \langle D^2 f(\vec x_0+t\vec v)\vec v, \vec v \rangle.
\end{align*}
Usando la formula di Taylor con resto di Lagrange per le funzioni di una variabile vista nel 
corso di Analisi Matematica Uno, si ottiene
\[
  g(t) = g(0) + g'(0) t + \frac 1 2 g''(\bar t(\vec x)) t^2
\]
per un qualche $\bar t(\vec x)\in (0,t)$. 
Abbiamo quindi 
\[
  f(\vec x) = f(\vec x_0) + \langle \nabla f(x_0), \vec x - \vec x_0\rangle 
  + \frac 1 2 \langle D^2 f(\vec x_0 + \bar t(\vec x)(\vec x - \vec x_0))(\vec x-\vec x_0), \vec x-\vec x_0\rangle.
\]
Visto che per $\vec x \to \vec x_0$ si ha $\bar t(\vec x)\to 0$,
per la continuità delle derivate seconde
si ha 
\[
  D^2 f(\vec x_0 + \bar t(\vec x)(\vec x - \vec x_0))
  \to D^2 f(\vec x_0)
\]
e dunque segue la tesi.
\end{proof}